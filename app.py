...
UI_LANG_MAP = {
    ...
    'expander_business_summary': "📖 คลิกเพื่ออ่านสรุปธุรกิจ",
    ...
    'tab_scan_market': "🚀 สแกนคัดหุ้นทั้ง 3 ตลาด",
}
...
# 2. Custom CSS สำหรับปรับหน้าตาบนมือถือและ Desktop
st.markdown(
    f"""
    <style>
/* 1. สไตล์พื้นฐาน (Global Styles) */
.block-container {{
...
# ============================================================
# ดึงรายชื่อหุ้นทั้งหมดจากทั้ง 3 ตลาด (NASDAQ, NYSE, AMEX) 7,000+ ตัว
# ============================================================
@st.cache_data(ttl=86400) # Cache 24 ชั่วโมง
...
def get_company_info(ticker):
    ...
    try:
        ...
        english_summary = info.get('longBusinessSummary', 'N/A')
...
def create_ta_chart(df, ticker, res_data):
    """สร้างกราฟ Interactive Candlestick พร้อมวาดเส้นแนวรับ-ต้าน และตีเทรนลายออโต้"""
    ...
    # ส่วนตีเทรนลายออโต้ (Linear Regression)
    days_for_fit = 40
    end_idx = len(df)
    ...
    try:
        ...
        fig.add_trace(go.Scatter(x=fit_dates, y=y_high_fit, line=dict(color='goldenrod', width=2), name='เทรนเเนวต้านล่าสุด'))
        fig.add_trace(go.Scatter(x=fit_dates, y=y_low_fit, line=dict(color='mediumturquoise', width=2), name='เทรนเเนวรับล่าสุด'))
    except Exception:
        pass
    ...
    try:
        ...
        if is_green and has_vol:
            ...
            if info_mode:
                ...
                if co_info_dict:
                    res_data['longName'] = co_info_dict['longNameEn']
            ...
            if res:
                ...
                with st.expander(UI_LANG_MAP['expander_business_summary'], expanded=True):
                    if english_summary != 'N/A':
                         st.markdown(f'<div class="biz-summary"><b>[Business Summary]</b><br>{english_summary}</div>', unsafe_allow_html=True)
                    else:
...
# ================= TAB 2: สแกนทั้ง 3 ตลาด (7,000+ หุ้น) =================
with tab2:
    ...
    if scan_btn:
        ...
        # ดึงรายชื่อหุ้นทั้งหมดจากทั้ง 3 ตลาด
        ...
        if results:
            ...
            st.download_button(
                ...
                data=csv_bytes_scan,
                file_name=f'us_market_scan_{time_stamp_scan}.csv',
                mime='text/csv'
            )
